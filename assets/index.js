      ...
      (position) => {
        ...
        axios
          ...
          .then((res) => {
            ...
            document.querySelector(
              check ? "#locationButton #timer" : "#locationButton1 #timer1",
            ).innerHTML = `<lottie-player class='handRight' id='handRight' src='assets/images/handRight.json' background='transparent' speed='1' autoplay loop style='width: 50px; height: 50px'></lottie-player><span id="countText"></span>`;

            let countdown = setInterval(() => {
              document.querySelector(
                check
                  ? "#locationButton #timer #countText"
                  : "#locationButton1 #timer1 #countText",
              ).innerText = count;
              count--;
